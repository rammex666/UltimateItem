...
public class YamlFiles {
    ...
    private static void loadFile(String fileName, String folder) {
        if(folder == null){
            file = new File(UltimateItem.instance.getDataFolder(), fileName + ".yml");
            if (!file.exists()) {
                file.getParentFile().mkdirs();
                UltimateItem.instance.saveResource(fileName + ".yml", false);
            }
        } else {
            file = new File(UltimateItem.instance.getDataFolder() + "/" + folder, fileName + ".yml");
            if (!file.exists()) {
                file.getParentFile().mkdirs();
                UltimateItem.instance.saveResource(folder+"/"+fileName + ".yml", false);
            }
        }
